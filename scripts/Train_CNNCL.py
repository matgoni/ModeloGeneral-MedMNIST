...
"""Script para entrenar un modelo EfficientNet-B0 como clasificador general de MedMNIST.
...
def construir_modelo(total_clases: int, usar_pesos_imagenet: bool = False) -> nn.Module:
    class CNN(nn.Module):
        def __init__(self, num_classes):
            ...
            # calcular tamaño final automáticamente
            with torch.no_grad():
                x = torch.zeros(1, 3, 28,28)
                x = self.features(x)
                self.num_features = x.shape[1] * x.shape[2] * x.shape[3]
...
def parsear_argumentos() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Entrena una CNN clásica como modelo general para MedMNIST."
    )
    parser.add_argument(
        "--datasets",
        nargs="+",
    ...
    parser.add_argument(
        "--salida",
        type=Path,
        default=Path("resultados") / "reporte_cnn_general.json",
